...
public static class SetupClassroomScene
{
    const string SCENE_PATH = "Assets/Scenes/StudentPages/ClassroomScene.unity";
    const string BG_PATH = "Assets/Images/Student/StudentHubBG.png";

    static TMP_FontAsset menuFont;

    [MenuItem("Tools/Setup Classroom Scene")]
    public static void Run()
    {
        ...
        // ── Build hierarchy (same order as StudentHub) ──
        BuildBackground(ct);
        ...
        EditorUtility.DisplayDialog("Done",
            "ClassroomScene rebuilt!\n\n" +
            "  Background (parallax + mouse)\n" +
            "  EmberOverlay (floating embers)\n" +
            "  ContentPanel (student scroll list)\n" +
            "  title (class name)\n" +
            "  ClassCode\n" +
            "  BottomBar (Back · Settings gear)\n" +
            "  FadeOverlay (intro fade)\n\n" +
            "Scene added to Build Settings.",
            "OK");
    }

    // ═══════════════════════════════════════════════
    //  BACKGROUND  (exact copy of StudentHub pattern)
    // ═══════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════
    //  CONTENT PANEL  (dark translucent + scroll)
    // ═══════════════════════════════════════════════
    ...
    static void BuildContentPanel(Transform canvas)
    {
        ...
        // Class code inside panel (right-aligned)
        ...
        // Scroll area (below the header row)
    ...
    // ═══════════════════════════════════════════════
    //  TITLE  (gold gradient, matches StudentHub exactly)
    // ═══════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════
    //  NO-STUDENTS TEXT  (matches StudentHub NoClassesText)
    // ═══════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════
    //  BOTTOM BAR  (same layout as StudentHub: Back · divider)
    // ═══════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════
    //  SETTINGS GEAR + PANEL  (exact copy from StudentHub)
    // ═══════════════════════════════════════════════
    ...
    static void BuildSettingsUI(Transform canvas)
    {
        ...
        // Settings panel (popup above gear)
    ...
    // ═══════════════════════════════════════════════
    //  WIRE: ClassroomManager
    // ═══════════════════════════════════════════════

    static void WireClassroomManager(GameObject canvasGO)
    {
        var mgr = canvasGO.AddComponent<ClassroomManager>();
        var so = new SerializedObject(mgr);
        var t = canvasGO.transform;

        var titleT = FindDeep(t, "title");
        if (titleT != null) so.FindProperty("classNameLabel").objectReferenceValue = titleT.GetComponent<TMP_Text>();

        var codeT = FindDeep(t, "ClassCode");
        if (codeT != null) so.FindProperty("classCodeLabel").objectReferenceValue = codeT.GetComponent<TMP_Text>();

        var containerT = FindDeep(t, "StudentListContainer");
        if (containerT != null) so.FindProperty("studentListContainer").objectReferenceValue = containerT;

        var countT = FindDeep(t, "StudentCount");
        if (countT != null) so.FindProperty("studentCountLabel").objectReferenceValue = countT.GetComponent<TMP_Text>();

        var noStudentsT = FindDeep(t, "NoStudentsText");
        if (noStudentsT != null) so.FindProperty("noStudentsText").objectReferenceValue = noStudentsT.gameObject;
        ...
        // Wire Back button onClick → ClassroomManager.GoBack
        ...
        if (backT != null)
        {
            ...
            if (btn != null)
            {
                ...
                var action = System.Delegate.CreateDelegate(
                    typeof(UnityEngine.Events.UnityAction), mgr, "GoBack") as UnityEngine.Events.UnityAction;
                if (action != null)
    ...
    // ═══════════════════════════════════════════════
    //  WIRE: StudentHubAnimator (same intro as Hub)
    // ═══════════════════════════════════════════════
    ...
    static void EnsureAudioManager()
    {
        ...
        if (musicGuids.Length > 0)
        {
            ...
            so.FindProperty("backgroundMusic").objectReferenceValue = AssetDatabase.LoadAssetAtPath<AudioClip>(clipPath);
        }
    ...
    // ═══════════════════════════════════════════════
    //  SIBLING ORDER  (same as StudentHub)
    // ═══════════════════════════════════════════════
    ...
    static void AddToBuildSettings(string scenePath)
    {
        ...
        Debug.Log("[SetupClassroomScene] Added " + scenePath + " to Build Settings.");
    }
